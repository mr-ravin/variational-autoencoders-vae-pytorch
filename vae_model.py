...
class VAE(nn.Module): # input image size: 3 x 128 x 128, here 3 represents number of channels i.e. r,g,b
    def __init__(self, enc_input_channels=3, enc_output_channels=32, latent_dim=32,mode="analysis"):
        super(VAE,self).__init__()
        self.mode = mode
        self.encoder_conv1 = nn.Conv2d(enc_input_channels, enc_output_channels, kernel_size=3,stride=1)
        self.encoder_conv2 = nn.Conv2d(enc_output_channels, enc_output_channels*2, kernel_size=5,stride=3)
        self.encoder_conv3 = nn.Conv2d(enc_output_channels*2, enc_output_channels*4, kernel_size=5,stride=3)
        self.encoder_conv4 = nn.Conv2d(enc_output_channels*4, enc_output_channels*8, kernel_size=5,stride=3)
        self.encoder_conv5 = nn.Conv2d(enc_output_channels*8, enc_output_channels*16, kernel_size=3,stride=1)
        self.fc1 = nn.Linear(enc_output_channels*16, 128)
        self.fc_mean = nn.Linear(128, latent_dim)
        self.fc_log_var = nn.Linear(128, latent_dim)
        self.fc2 = nn.Linear(latent_dim, enc_output_channels*16)
        self.decoder_deconv5 = nn.ConvTranspose2d(
            in_channels=enc_output_channels*16, out_channels=enc_output_channels*8, kernel_size=3, 
            stride=1,
        )
        self.decoder_deconv4 = nn.ConvTranspose2d(
            in_channels=enc_output_channels*8, out_channels=enc_output_channels*4, kernel_size=5, 
            stride=4,
        )

        self.decoder_deconv3 = nn.ConvTranspose2d(
            in_channels=enc_output_channels*4, out_channels=enc_output_channels*2, kernel_size=5, 
            stride=3,
        )
        self.decoder_deconv2 = nn.ConvTranspose2d(
            in_channels=enc_output_channels*2, out_channels=enc_output_channels, kernel_size=6, 
            stride=3,
        )
        self.decoder_deconv1 = nn.ConvTranspose2d(
            in_channels=enc_output_channels, out_channels=enc_input_channels, kernel_size=3, 
            stride=1,
        )
